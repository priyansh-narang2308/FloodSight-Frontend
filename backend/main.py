...
# sets the basic config for the login info
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# initalize the gemini cliet
genai.configure(api_key=os.getenv("GEMINI_API_KEY"))

app = FastAPI(
    title="Flood Detection API",
    description="API for flood detection assessment using Gemini AI ",
    version="1.0.0",
)


# configure the cors middlerware
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
class AnalysisRespons(BaseModel):
    success: bool
    risk_level: str
    description: str
